...
//    private record UserInput(String text) {}
    ...
    @GetMapping("/end")
    private Mono<String> getFinalFeedback(WebSession session) {
    	List<Content> history = (List<Content>) session.getAttributes().get(HISTORY_KEY);
    	
    	if (history == null) {
            return Mono.just("No History");
        }
    	
        // Final prompt to get overall feedback
        return interviewService.getNextResponse(
            history, 
            "Provide a comprehensive, overall feedback report on my interview performance, covering technical depth, communication, and key strengths/weaknesses from the entire conversation history."
        )
        .map(feedback -> {
            // Clean up session state
        	session.getAttributes().remove(HISTORY_KEY);
            return "--- INTERVIEW COMPLETE: FINAL FEEDBACK ---\n" + feedback;
        });
    }
}